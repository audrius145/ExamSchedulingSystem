...
public class ExamListViewModel
{
    ...
    public ObservableList<ExamViewModel> updateFilter(ArrayList<Exam> examsBy)
    {
        ArrayList<Exam> exams = new ArrayList<>();
        for (int i = 0; i < examsBy.size(); i++)
        {
            exams.add(examsBy.get(i));
        }
        list.clear();
        for (int i = 0; i < exams.size(); i++)
        {
            ExamViewModel examViewModel = new ExamViewModel(exams.get(i));
            list.add(examViewModel);
        }
        return list;
    }
